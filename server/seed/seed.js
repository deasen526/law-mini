...
const categories = [
  { name: '劳动维权', icon: '⚖️', sortOrder: 1, isActive: true },
  { name: '租房纠纷', icon: '🏠', sortOrder: 2, isActive: true },
  { name: '法律文书', icon: '📄', sortOrder: 3, isActive: true },
  { name: '年度服务', icon: '⭐', sortOrder: 4, isActive: true },
];
...
const products = [
  {
    ...
    process: [
      ...
      { step: 2, title: '案情分析', desc: '24小时内法律顾问分析你的案件' },
      { step: 3, title: '定制文书', desc: '我们帮你起草仲裁申请书和证据清单' },
      { step: 4, title: '立案指导', desc: '告诉你去哪里立案、带什么材料' },
      { step: 5, title: '开庭辅导', desc: '开庭前电话模拟，让你知道怎么说话' },
    ...
    cases: [
      ...
      { title: '销售员小李', desc: '试用期被辞退没给补偿，我们陪他走完仲裁，最终调解拿回赔偿。', result: '调解拿回12,000元' },
    ],
  ...
  },
  {
    ...
    process: [
      ...
      { step: 2, title: '生成报告', desc: '即时生成分析报告' },
      { step: 3, title: '人工复核', desc: '法律顾问复核关键结论' },
      { step: 4, title: '获取报告', desc: '收到完整分析报告' },
    ],
    cases: [],
  ...
  },
  {
    ...
    process: [
      ...
      { step: 4, title: '律师操盘', desc: '律师全流程代理' },
    ],
    cases: [],
  ...
  },
  {
    ...
    process: [
      ...
      { step: 2, title: '发催告函', desc: '用标准催告函正式通知房东退款' },
      { step: 3, title: '逐级投诉', desc: '如房东不理，按指引逐级投诉' },
      { step: 4, title: '小额诉讼', desc: '最后手段：自己去法院小额诉讼' },
    ],
    cases: [],
  ...
  },
  {
    ...
    originalPrice: 79900,
    coverImage: '',
    scenario: '需要正式的法律文书震慑对方、表明你的严肃立场。',
    ...
    process: [
      ...
      { step: 3, title: '确认内容', desc: '发给你确认内容无误' },
  ...
  },
  {
    ...
    originalPrice: 29900,
    coverImage: '',
    scenario: '你的法律问题不是一次性的——换工作、租房、消费维权……一年总会遇到几次。',
    ...
    process: [
      ...
      { step: 2, title: '加专属顾问', desc: '系统分配专属法律顾问微信' },
      { step: 3, title: '随时咨询', desc: '有问题随时在微信上问' },
    ],
    cases: [],
    guarantee: '7天内未使用可申请退款。',
    sortOrder: 1,
...
async function seed() {
  try {
    // 检查已有数据
    const catCount = Category.all().length;
    if (catCount === 0) {
      Category.bulkCreate(categories);
      console.log('✅ 4 个分类已创建');
    } else {
      console.log('⏭️  分类已存在，跳过');
    }
    ...
    console.log(`📂 数据文件位置: server/data/`);
  } catch (err) {
